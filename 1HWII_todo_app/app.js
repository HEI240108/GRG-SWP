...
function todoTextExists(text) {
    return state.todos.some((_) => _.text === text);
}
function getTodo(text) {
    return state.todos.find((_) => _.text == text);
}
...
function createTodoDelBtn() {
    return '<button onClick="onRemoveTodo(event)">delete</button';
}
function createTodoCheckBox(todo) {
    return `
    <input type="checkbox"
      ${todo.completed ? 'checked' : ''} onClick="onToggleTodoCompleted(event)">
    `;
}
// 5. RENDER FN
...
function render() {
    todoList$.innerHTML = state.todos
        ...
        .map(createTodoItem)
        .join('\n');
    todoListDone$.innerHTML = state.todos
        .filter((_) => _.completed)
        .map(createTodoItem)
        .join('\n');
}
...
function onToggleTodoCompleted(evt) {
    const text = evt.target.parentElement.querySelector('p').textContent;
    toggleTodoCompleted(text);
    render();
    // console.log(text);
}
function onRemoveTodo(evt) {
    const text = evt.target.parentElement.querySelector('p').textContent;
    removeTodo(text);
    render();
    // update state
